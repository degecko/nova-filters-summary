<template>
  <div class="filters-summary-card" v-if="activeFilters.length">
    ...
    <div class="flex flex-wrap">
      <div
        class="fsc-filter flex bg-white shadow px-1 py-1 mr-2 mb-2"
      ...
      >
        ...
        <div class="ml-2">
          <div
            class="remove-filter flex align-items-center justify-center font-bold bg-40 cursor-pointer hover:text-white hover:bg-90"
...
export default {
  ...
  methods: {
    ...
    getActiveFilters(
      filters = this.$store.getters[`${this.resourceName}/filters`]
    ) {
      this.activeFilters = _.cloneDeep(filters)
          ...
          if (filter.component === "boolean-filter") {
            if (typeof filter.currentValue === "object") {
              for (let id in filter.currentValue) {
                if (
                  filter.currentValue.hasOwnProperty(id) &&
                  filter.currentValue[id]
                ) {
                  return true;
                }
              }
        ...
        .map((filter) => {
          if (Nova.filtersSummaryResolvers[filter.component]) {
            filter.summary =
              Nova.filtersSummaryResolvers[filter.component](filter);
          } else if (filter.component === "select-filter") {
            filter.summary = filter.options.find(
              (o) => this.nin(o.value) === this.nin(filter.currentValue)
            )?.name;
            ...
            filter.summary = enabledValues.length
              ? enabledValues.join(", ")
              : "<em>none</em>";
